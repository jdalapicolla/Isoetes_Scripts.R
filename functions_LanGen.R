...
summary_stat = function(datasets, names){
  ...
  write.csv(FST_stat$global, file = paste0("./Results_Distance/FST_global_", project_name,"_", names[i], ".csv"))
  ...
  pdf(paste0("Results_Distance/Estimates_Population_Differentiation_", names[i], ".pdf"), onefile = F)
  print(plot(pp))
  dev.off()
  ...
  D = as.matrix(D)
  write.csv(D, file = paste0("Results_Distance/D_groups_", project_name ,"_", names[i],".csv"))
  
  GST_N = pairwise_Gst_Nei(input2, linearized = FALSE)
  GST_N = as.matrix(GST_N)
  write.csv(GST_N, file = paste0("Results_Distance/GST_N_groups_",project_name ,"_", names[i], ".csv"))
  
  GST_H = pairwise_Gst_Hedrick(input2, linearized = FALSE)
  GST_H = as.matrix(GST_H)
  write.csv(GST_H, file = paste0("Results_Distance/GST_H_groups_", project_name ,"_", names[i], ".csv"))
  
  #by populations and global to other indexes. See the help for poppr() for acronyms' meanings:
  sum_stat = poppr(input2)
  write.csv(sum_stat, file = paste0("Results_Distance/Sum_Stat_groups_", project_name ,"_", names[i], ".csv"))
}
}
